...
class ControllerNode(Node):
    ...
    def publish_motor_commands(self, topic_name, msg):
        publisher = self.get_motor_publisher(topic_name)
        publisher.publish(msg)

    def publish_wheel_commands(self, topic_name, msg):
        publisher = self.get_wheel_publisher(topic_name)
        publisher.publish(msg)

    def get_motor_publisher(self, topic_name):
        # 이미 생성된 발행자가 있으면 재사용
    ...
    def get_wheel_publisher(self, topic_name):
        # 이미 생성된 발행자가 있으면 재사용
        if topic_name not in self.custom_publishers:
            self.custom_publishers[topic_name] = self.create_publisher(CocelWheelCmd, topic_name, 10)
        return self.custom_publishers[topic_name]
